...
for ii = 1:length(whichDataTypes)
    switch lower(whichDataTypes{ii})
        ...
        case 'sf4'
            ...
            if ~exist(fullfile(dfdRootPath, 'analysis', 'data','s99_withoutBB'),7)
                    mkdir(fullfile(dfdRootPath, 'analysis', 'data','s99_withoutBB')); end
    end
end
    ...
    if s == 99
        for f = 1:length(fnames)
            ...
            if any(strcmp(urlStr{f},SF4_withoutBB)) 
                savePth = fullfile(dfdRootPath, 'analysis', 'data','s99_withoutBB');
            end
        ...
        for f = 1:length(fnames)
            if ~strcmp(fnames{f},'_denoisedData_control')
                ...
                try
                    ...
                    warning('(dfdDownloadsampledata): Cannot download version 2 of file, so will default to version 1')
                    readPth  = fullfile(dirProject, urlStr{f}, '?action=download&version=1');
                    websave(writePth,readPth);
                end
