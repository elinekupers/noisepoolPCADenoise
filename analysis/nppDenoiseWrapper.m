function nppDenoiseWrapper(subjects, howToDenoise)
%
% nppDenoiseWrapper(subjects, howToDenoise)
% ---------------------------------------------------------------- 
% INPUTS:
% -----------------
...
if isempty(dir(fullfile(nppRootPath, 'analysis', 'data', 's*.mat')))
    error('No data were found. Use nppDownloadSampleData')
end
...
% Preprocessing parameters to remove bad channels and epochs (see nppPreprocessData)
...
for whichSubject = subjects
    ...
    tmp = load(sprintf(fullfile(nppRootPath, 'analysis', 'data', 's%02d_sensorData.mat'),whichSubject)); sensorData = tmp.sensorData;
    tmp = load(sprintf(fullfile(nppRootPath, 'analysis', 'data', 's%02d_conditions.mat'),whichSubject)); conditions = tmp.conditions;
    
    if removeMsEpochs
    tmp = load(sprintf(fullfile(nppRootPath, 'analysis', 'data', 'eye', 's%02d_epochswithms.mat'),whichSubject)); msepochidx = cat(1,tmp.allEpochsWithMS{:}); end %#ok<UNRCH>
...
if howToDenoise == 4
    ...
    for nrControl = nrControlModes   
        ...
        if (0 <= nrControl) && (nrControl <= 4)
            ...
            if saveDenoisedts; save(sprintf(fullfile(nppRootPath, 'analysis', 'data', 's%02d_denoisedts.mat'),whichSubject),'denoisedts_bb'); end
        ...
        % ------------------ Define file name ---------------------------
        fname = sprintf(fullfile(nppRootPath,'analysis','data', ['s%02d_denoisedData' postFix]), whichSubject);   
