...
function nppFindOptimalNoisepool(subjects)
...
if isempty(fullfile(nppRootPath, 'data'));
...
for whichSubject = subjects
    ...
    tmp = load(sprintf(fullfile(nppRootPath, 'exampleAnalysis','data', 's0%d_sensorData.mat'),whichSubject)); sensorData = tmp.sensorData;
    tmp = load(sprintf(fullfile(nppRootPath, 'exampleAnalysis','data', 's0%d_conditions.mat'),whichSubject)); conditions = tmp.conditions;
...
%     
%     if use3Channels
%         fname = sprintf(fullfile(nppRootPath,'data','s0%d_denoisedData_w3chan'),whichSubject);
%     else
%         fname = sprintf(fullfile(nppRootPath,'data','s0%d_denoisedData'),whichSubject);
%     end
    fname = sprintf(fullfile(nppRootPath,'exampleAnalysis','data','s0%d_noisepoolresults'),whichSubject);
